package com.zhongzhou.Excavator.ETL.loader.publish;
...
@Component( ETLLoaderList.load_saleOrderState_to_ec  )
@Scope("prototype") 
public class SaleOrderState  extends SimpleBasicNode  implements Loader {
	
	@Resource(name=DAOBeanNameList.mongo_nc_report_saleOrder)
	SaleOrderDAO mongoOuterSaleOrderDAO;
	...
	public void doLoad() throws ETLRuntimeException {
		...
		if( this.saleOrderStates != null && this.saleOrderStates.size() > 0 ){
			...
			mongoOuterSaleOrderDAO.updateRerports2NotLastVersion( orderIds );
			mongoOuterSaleOrderDAO.insertReports( mongoDatas );
		}
	}
}
